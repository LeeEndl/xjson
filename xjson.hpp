...
#include <string>
#include <vector>
std::vector<std::string> index(std::string source, std::string indexOf) {
	std::vector<std::string> buffer;
	size_t slen = source.length(), ilen = indexOf.length(), i = 0, k = 0;
	...
	buffer.push_back(source.substr(k, i - k));
	return buffer;
}
